              ...
              {row.cells.map((cell, index) => {
                ...
                return (
                  <td
                    key={key}
                    {...restCellProps}
                    className={`px-4 py-2 text-center text-gray-600 ${index === 0 ? "font-bold sticky left-0 bg-slate-200 border-r border-b border-black" : "border border-black"}`}
                  >
                    {value ? value : "-"}
                  </td>
                );
          ...
          return (
            ...
            <tr key={key} {...restFooterGroupProps}>
              {footerGroup.headers.map((column, index) => {
                ...
                return (
                  <td key={key} {...column.getFooterProps} className={`px-2 py-2 font-bold text-center text-gray-600 ${index === 0 ? "font-bold sticky left-0 bg-slate-200 border-r border-t border-b border-black" : "border border-black"}`}>
